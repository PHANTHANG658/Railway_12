...
SELECT b.DepartmentID,b.DepartmentName ,COUNT(a.DepartmentID) as 'so nhan vien'  FROM `Account`  a 
...
-- 7 THONG KE MOI QUESTION SU CHO BAO NHEIU EXEM-->>>> COUNT (QUESTION) 
SELECT e.ExamID, COUNT(q.questionID) as'sl' FROM question q RIGHT JOIN examquestion eq 
...
SELECT		g.GroupID, COUNT(ga.AccountID) AS 'SO LUONG'
 FROM  `Account` ga JOIN 	`Group` G ON	ga.GroupID = G.GroupID
GROUP BY	g.GroupID 
...
-- 17 - Question 17:
-- a) Lấy các account id 1
SELECT 	a.FullName
FROM `Account` a
JOIN GroupAccount  ga ON a.AccountID = ga.AccountID
WHERE ga.GroupID = 1;
-- b) Lấy các account id 3
SELECT 	a.FullName
FROM `Account` a
JOIN GroupAccount ga ON A.AccountID = ga.AccountID
WHERE ga.GroupID = 3;
